...
//            whereQueryStr.append(" AND EGCL_COLLECTIONHEADER.STATUS =:searchStatus");
//        }
        if (applicationTypeId != null && applicationTypeId !=-1) {
        	   whereQueryStr.append(" AND EGBPA_APPLICATION.APPLICATIONSUBTYPE =:applicationTypeId"); 
        }else if(appTypeList !=null && !appTypeList.isEmpty())  {
        ...
        if (applicationTypeId != null && applicationTypeId !=-1) {
        	aggrQuery.setLong("applicationTypeId", applicationTypeId);
            rebateQuery.setLong("applicationTypeId", applicationTypeId);
	     }
...
//        }
	...
	public Object getCollectionSummaryReportForUrbanOC(Date fromDate, Date toDate, String paymentMode, String source,
			String glCode, Integer branchId, Long applicationTypeId, List<Long> appTypeList) {
		final SimpleDateFormat fromDateFormatter = new SimpleDateFormat("yyyy-MM-dd 00:00:00");
        final SimpleDateFormat toDateFormatter = new SimpleDateFormat("yyyy-MM-dd 23:59:59");
        final StringBuilder defaultQueryStr = new StringBuilder(500);
