...
@Service
public class GeneralStairRural extends FeatureProcess {
	private static final Logger LOG = Logger.getLogger(GeneralStairRural.class);
	...
	public Plan process(Plan plan) {
		if (!CDGAdditionalService.isFeatureValidationRequired(plan, GeneralStairRural.class))
			return plan;
		// validate(planDetail);
				...
				if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
					...
					if (valid) {
						setReportOutputDetailsFloorStairWise(plan,
								...
								CDGAdditionalService.viewLenght(plan, minimumWidth),
								CDGAdditionalService.viewLenght(plan, minWidth), Result.Accepted.getResultVal(),
						...
						setReportOutputDetailsFloorStairWise(plan,
								...
								String.format(WIDTH_LANDING_DESCRIPTION, generalStair.getNumber(), landing.getNumber()),
								CDGAdditionalService.viewLenght(plan, minimumWidth),
								CDGAdditionalService.viewLenght(plan, minWidth), Result.Not_Accepted.getResultVal(),
								scrutinyDetailLanding);
					}
				}
			} else {
	...
	private void validateHeightOfRiser(Plan pl, Block block, Floor floor, ScrutinyDetail scrutinyDetailHeightRise,
		...
		for (org.egov.common.entity.edcr.GeneralStair generalStair : generalStairs) {
			...
			if (valid)
				setReportOutputDetailsFloorStairWise(pl,
						CDGAdditionalService.getByLaws(occupancyTypeHelper, CDGAConstant.STAIRCASE),
						floor.getNumber().toString(),
						String.format(HEIGHT_OF_RISER_DESCRIPTION, generalStair.getNumber()),
						CDGAdditionalService.viewLenght(pl, raiserHeightexpected),
						CDGAdditionalService.viewLenght(pl, raiserHeightProvided), Result.Accepted.getResultVal(),
						scrutinyDetailHeightRise);
			else
				setReportOutputDetailsFloorStairWise(pl,
						CDGAdditionalService.getByLaws(occupancyTypeHelper, CDGAConstant.STAIRCASE),
						...
						String.format(HEIGHT_OF_RISER_DESCRIPTION, generalStair.getNumber()),
						CDGAdditionalService.viewLenght(pl, raiserHeightexpected),
						CDGAdditionalService.viewLenght(pl, raiserHeightProvided), Result.Not_Accepted.getResultVal(),
						scrutinyDetailHeightRise);
		...
		if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
			...
			if (valid) {
				setReportOutputDetailsFloorStairWise(plan,
						...
						CDGAdditionalService.viewLenght(plan, minimumWidth),
						CDGAdditionalService.viewLenght(plan, minFlightWidth), Result.Accepted.getResultVal(),
						scrutinyDetail2);
			} else {
				setReportOutputDetailsFloorStairWise(plan,
						CDGAdditionalService.getByLaws(mostRestrictiveOccupancyType, CDGAConstant.STAIRCASE), value,
						String.format(WIDTH_DESCRIPTION, generalStair.getNumber(), flight.getNumber()),
						CDGAdditionalService.viewLenght(plan, minimumWidth),
						CDGAdditionalService.viewLenght(plan, minFlightWidth), Result.Not_Accepted.getResultVal(),
						scrutinyDetail2);
			}
		}
				...
				if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
					...
					if (valid) {
						setReportOutputDetailsFloorStairWise(plan,
								...
								CDGAdditionalService.viewLenght(plan, requiredTread),
								CDGAdditionalService.viewLenght(plan, minTread), Result.Accepted.getResultVal(),
								scrutinyDetail3);
					} else {
						setReportOutputDetailsFloorStairWise(plan,
								CDGAdditionalService.getByLaws(mostRestrictiveOccupancyType, CDGAConstant.STAIRCASE),
								value, String.format(TREAD_DESCRIPTION, generalStair.getNumber(), flight.getNumber()),
								CDGAdditionalService.viewLenght(plan, requiredTread),
								CDGAdditionalService.viewLenght(plan, minTread), Result.Not_Accepted.getResultVal(),
								scrutinyDetail3);
					}
				}
			} else {
