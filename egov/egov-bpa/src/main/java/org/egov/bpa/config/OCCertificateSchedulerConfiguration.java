...
public class OCCertificateSchedulerConfiguration extends QuartzSchedulerConfiguration {
    ...
    public SchedulerFactoryBean bpaOcCertificateScheduler(DataSource dataSource) {
        SchedulerFactoryBean bpaOcScheduler = createScheduler(dataSource);
        bpaOcScheduler.setSchedulerName("bpa-oc-cer-scheduler");
        ...
        if (bpaApplicationSettings.finalCertificateSchedulerEnabledForOC())
            triggers.add(ocFinalCertificateCronTrigger().getObject());
        bpaOcScheduler.setTriggers(triggers.toArray(new Trigger[triggers.size()]));
        return bpaOcScheduler;
    }
    @Bean
    public CronTriggerFactoryBean ocFinalCertificateCronTrigger() {
        CronTriggerFactoryBean finalCertificateCron = new CronTriggerFactoryBean();
        finalCertificateCron.setJobDetail(ocFinalCertificateJobDetail().getObject());
        finalCertificateCron.setGroup("OC_CERATIFICATE_TRIGGER_GROUP");
        finalCertificateCron.setName("OC_FINAL_CERTIFICATE_TRIGGER");
        finalCertificateCron.setCronExpression(bpaApplicationSettings.getValue("bpa.oc.final.certificate.job.cron"));
        finalCertificateCron.setMisfireInstruction(MISFIRE_INSTRUCTION_DO_NOTHING);
        return finalCertificateCron;
    }

    @Bean("ocFinalCertificateJob")
    public OcFinalCertificateJob finalCertificateJob() {
    ...
    public JobDetailFactoryBean ocFinalCertificateJobDetail() {
        ...
        scheduleAppointmentJobDetail.setGroup("OC_CERATIFICATE_TRIGGER_GROUP");
        scheduleAppointmentJobDetail.setName("OC_FINAL_CERTIFICATE_JOB");
        scheduleAppointmentJobDetail.setDurability(true);
        scheduleAppointmentJobDetail.setJobClass(OcFinalCertificateJob.class);
