...
function uniformBoundaryNew(selectedBndryId, hierarchy, toHeirarchy){
	...
	$.ajax({
		...
		success : function(response) {
			...
			if(selectedBndryId == 'REVENUEAreaCategory'){
				var planZoneOrLocation = $("#planZoneOrLocation").val();
				if(typeof planZoneOrLocation != "undefined") {
                	if (planZoneOrLocation != "") {
                		$("#REVENUEZoneorLocation option").each(function() {
        					if($(this).text().toUpperCase() == planZoneOrLocation.toUpperCase()) {
        						$(this).attr('selected', 'selected');
        						$("#REVENUEZoneorLocation").change();
        						$('#REVENUEZoneorLocation').attr("disabled", "disabled");
        					}                        
        			    });
                	}
				}			
			}
			if(selectedBndryId == 'REVENUEZoneorLocation'){
				var planSectorOrVillage = $("#planSectorOrVillage").val();
				if(typeof planSectorOrVillage != "undefined") {
                	if (planSectorOrVillage != "") {
                		$("#REVENUESectorsorVillages option").each(function() {
	       					 if($(this).text().toUpperCase() == planSectorOrVillage.toUpperCase()) {
	       						$(this).attr('selected', 'selected');
	       						$('#REVENUESectorsorVillages').attr("disabled", "disabled");
	       					 }                        
                		});
                	}
				}				
