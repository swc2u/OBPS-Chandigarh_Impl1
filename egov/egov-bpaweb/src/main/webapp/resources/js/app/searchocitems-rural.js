$(document).ready(function() {
	$('#btnSearch').click(function() {
		var isValid = false;
		$('#searchOCRuralItemsForm').find(':input', ':select', ':textarea').each(function() {
			if ($(this).val()) {
				isValid = true;
				return false;
			} else
...
function graph() {
	$.ajax({
		type: "POST",
		url: "/bpa/application/searchOCItems/d/r",
		data: formdata
		,
...
function callAjaxSearch() {
	...
	$('.oc-task-rural-report-section').removeClass('display-hide');
	$("#search_oc_rural_items_table").dataTable({
		processing: true,
		serverSide: true,
		sort: true,
		filter: true,
		...
		"order": [[1, 'asc']],
		ajax: {
			url: "/bpa/application/searchOCItems/r",
			type: "POST",
			beforeSend: function() {
