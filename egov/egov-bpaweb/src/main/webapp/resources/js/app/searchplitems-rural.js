$(document).ready(function() {
	$('#btnSearch').click(function() {
		var isValid = false;
		$('#searchPLRuralItemsForm').find(':input', ':select', ':textarea').each(function() {
			if ($(this).val()) {
				isValid = true;
				return false;
			} else
...
function graph() {
	$.ajax({
		type: "POST",
		url: "/bpa/reports/searchPLItems/d/r",
		data: formdata
		,
...
function callAjaxSearch() {
	...
	$('.pl-task-report-section-rural').removeClass('display-hide');
	$("#search_pl_rural_items_table").dataTable({
		processing: true,
		serverSide: true,
		sort: true,
		filter: true,
		...
		"order": [[1, 'asc']],
		ajax: {
			url: "/bpa/reports/searchPLItemsRural",
			type: "POST",
			beforeSend: function() {
