$(document).ready(function() {
	$('#btnSearch').click(function() {
		var isValid = false;
		$('#searchPLRuralPendingItemsForm').find(':input', ':select', ':textarea').each(function() {
			if ($(this).val()) {
				isValid = true;
				return false;
			} else
...
function graph() {
	$.ajax({
		type: "POST",
		url: "/bpa/reports/searchPLPendingItems/d/r",
		data: formdata
		,
...
function showGraph(json) {
	...
	var chart = new CanvasJS.Chart("chartContainer", {
		...
		theme: "light2",
		title: {
			text: "DPC/ Plinth level Certificate -Urban"
		},
		axisY: {
			title: "Units",
			titleFontSize: 24,
...
function callAjaxSearch() {
	var viewurl = '/bpa/application/citizen/plinth-level-certificate/update/';
	$('.pl-pending-task-report-section-rural').removeClass('display-hide');
	$("#search_pl_pending_rural_items_table").dataTable({
		processing: true,
		serverSide: true,
		sort: true,
		filter: true,
		...
		"order": [[1, 'asc']],
		ajax: {
			url: "/bpa/reports/searchPLPendingItemsRural",
			type: "POST",
			beforeSend: function() {
