$(document).ready(function() {
	$('#btnSearch').click(function() {
		var isValid = false;
		$('#searchPLUrbanPendingItemsForm').find(':input', ':select', ':textarea').each(function() {
			if ($(this).val()) {
				isValid = true;
				return false;
			} else
...
function graph() {
	$.ajax({
		type: "POST",
		url: "/bpa/reports/searchPLPendingItems/d/u",
		data: formdata
		,
...
function callAjaxSearch() {
	...
	$('.pl-pending-task-report-section').removeClass('display-hide');
	$("#search_pl_pending_urban_items_table").dataTable({
		processing: true,
		serverSide: true,
		sort: true,
		filter: true,
		...
		"order": [[1, 'asc']],
		ajax: {
			url: "/bpa/reports/searchPLPendingItems",
			type: "POST",
			beforeSend: function() {
