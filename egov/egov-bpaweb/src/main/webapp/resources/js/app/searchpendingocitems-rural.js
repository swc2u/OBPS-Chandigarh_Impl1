$(document).ready(function() {
	$('#btnSearch').click(function() {
		var isValid = false;
		$('#searchOCPendingItemsRuralForm').find(':input', ':select', ':textarea').each(function() {
			if ($(this).val()) {
				isValid = true;
				return false;
			} else
...
function graph() {
	$.ajax({
		type: "POST",
		url: "/bpa/application/searchOCPendingItems/d/r",
		data: formdata
		,
...
function callAjaxSearch() {
	...
	$('.oc-rural-pending-report-section').removeClass('display-hide');
	$("#search_oc_pending_items_rural_table").dataTable({
		processing: true,
		serverSide: true,
		sort: true,
		filter: true,
		...
		"order": [[1, 'asc']],
		ajax: {
			url: "/bpa/application/searchOCPendingItems/r",
			type: "POST",
			beforeSend: function() {
