...
<div class="panel-body rejectionReason display-hide">
	<table class="table table-bordered  multiheadertbl"
		...
		<tbody>
		<c:choose>
			<c:when test="${not empty plinthLevelCertificate.rejectionReasonsTemp}">
				<c:forEach var="rejectionReason" items="${plinthLevelCertificate.rejectionReasonsTemp}" varStatus="rejectPCStatus">
					<tr>
						<td class="text-center">
						<form:hidden path="rejectionReasonsTemp[${rejectPCStatus.index}].pl" value="${plinthLevelCertificate.id}" />
						...
						<form:hidden path="rejectionReasonsTemp[${rejectPCStatus.index}].noticeCondition.type" value="PLREJECTIONREASONS" />
	...
	<table class="table table-bordered  multiheadertbl"
		...
		<c:choose>
			<c:when
					test="${not empty plinthLevelCertificate.additionalRejectReasonsTemp}">
				<c:forEach var="addnlPermitCondition" items="${plinthLevelCertificate.additionalRejectReasonsTemp}"
						   varStatus="addnlPCStatus">
					<tr>
						<td class="text-center">
						<form:hidden path="additionalRejectReasonsTemp[${addnlPCStatus.index}].pl" value="${plinthLevelCertificate.id}" />
			...
			<c:forEach var="addnlRejectionReason" items="${additionalRejectionReasons}">
				<tr>
					<td class="text-center">
					<form:hidden path="additionalRejectReasonsTemp[0].pl" value="${plinthLevelCertificate.id}" />
