...
<div class="panel-body display-hide">
	<c:if test="${not empty plinthLevelCertificate.parent.buildingDetail}">
		<c:forEach items="${plinthLevelCertificate.parent.buildingDetail}" var="bldg"
			...
			<c:if test="${not empty bldg.applicationFloorDetails}">
				<div
					...
					<div class="panel-title">
						<c:if test="${fn:length(plinthLevelCertificate.parent.buildingDetail) gt 1}">Block - ${bldg.name}</c:if>
						<spring:message code="lbl.plint.carpet.details" />
					</div>
					<c:if test="${fn:length(plinthLevelCertificate.parent.buildingDetail) gt 1}">
				...
				<div
					<c:if
                    test="${fn:length(plinthLevelCertificate.parent.buildingDetail) gt 1}">class="buildingDetailsToggle${bldgCounter.index} display-hide" </c:if>>
						...
						<tbody>
							...
							<c:forEach items="${bldg.applicationFloorDetails}"
								...
								<input type="hidden" value="${floorDetails.id}"
									...
									name="plinthLevelCertificate.parent.buildingDetail[0].applicationFloorDetails[${counter.index}].id" />
