...
<div class="panel-heading slide-history-menu-1">
	<div class="panel-title">
		<spring:message code="lbl.noc.wf.history" />
	</div>
	<div class="history-icon">
		<i class="fa fa-angle-up fa-2x" id="toggle-his-icon-1"></i>
	</div>
</div>
<div class="panel-body history-slide-1 display-hide">
	<div class="row add-margin hidden-xs visible-sm visible-md visible-lg header-color">
		<div class="col-sm-2 col-xs-6 add-margin">
			<spring:message code="lbl.noc.app.no" />
		</div>
		<div class="col-sm-2 col-xs-6 add-margin">
	...
	<c:choose>
		<c:when test="${!nocWorkflowHistory.isEmpty()}">
			<c:forEach items="${nocWorkflowHistory}" var="history" varStatus="appHisStatus">
				<div class="row add-margin">
					<div class="col-sm-2 col-xs-12 add-margin">
						<c:out value="${history.nocApplicationNumber}" />
					</div>
					<div class="col-sm-2 col-xs-12 add-margin">
...
<script>
	$('.slide-history-menu-1').click(
			function() {
				$('.history-slide-1').slideToggle();
				if ($('#toggle-his-icon-1').hasClass('fa fa-angle-down')) {
					$('#toggle-his-icon-1').removeClass('fa fa-angle-down')
							.addClass('fa fa-angle-up');
					//$('#see-more-link').hide();
				} else {
					$('#toggle-his-icon-1').removeClass('fa fa-angle-up')
