			...
			<c:choose>
				<c:when test="${not empty  checklists}">
					<c:forEach items="${checklists}" var="checklist"
						varStatus="status">
						<tr id="${checklist.code}" class="genDocs">
							<td class="view-content text-center" style="font-size: 97%;"><c:out value="${status.index+1}" /></td>
							<td class="view-content text-justify" style="font-size: 97%;"><c:out value="${checklist.description}"
									default="N/A" /></td>
							<td ><select 
							id="checklist${status.index}remark"
							name="checklist[${status.index}].comment"> 
							...
							<td ><textarea
								...
								id="checklist${status.index}comment"
								data-pattern="alphanumericspecialcharacters" maxlength="999"
								name="checklist[${status.index}].comment" /></td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
